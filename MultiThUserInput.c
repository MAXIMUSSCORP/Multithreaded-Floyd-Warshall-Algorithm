...
int main() {
    ...
    for (int i = 0; i < E; i++) {
        ...
        graph[u-1][v-1] = w;
        graph[v-1][u-1] = w;
        dist[u-1][v-1] = graph[u-1][v-1];
        dist[v-1][u-1] = graph[v-1][u-1];
    }
    ...
    sem_destroy(&dbLock); // Correctly destroy dbLock semaphore
    sem_destroy(&readCountLock); // Also destroy readCountLock semaphore
